export class DevicePointer {
	...
	static readonly isNone = globalThis.matchMedia?.("(any-pointer:none)").matches;

	/** coarse - The primary input mechanism includes a pointing device of limited accuracy,
	 * such as a finger on a touchscreen. */
	static readonly isCoarse = globalThis.matchMedia?.("(any-pointer:coarse)").matches;

	/** fine - The primary input mechanism includes an accurate pointing device, such as a mouse. */
	static readonly isFine = globalThis.matchMedia?.("(any-pointer:fine)").matches;
}
